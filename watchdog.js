...
export function cancel(id) {
  if (_timers.once.has(id)) {
    try {
      clearTimeout(_timers.once.get(id));
    } catch (_) {}
    _timers.once.delete(id);
    return true;
  }
  if (_timers.interval.has(id)) {
    try {
      clearInterval(_timers.interval.get(id));
    } catch (_) {}
...
function applyRuntime(r) {
  if (!r) return;
  if (r.loop) {
    if (typeof r.loop.baseMs === 'number') _runtime.loop.baseMs = r.loop.baseMs;
  ...
  if (r.thresholds) {
    if (typeof r.thresholds.bufferingMinMs === 'number') _runtime.thresholds.bufferingMinMs = r.thresholds.bufferingMinMs;
...
const _adapter = {
  isBuffering(c) {
    try {
      const ok = allTrue([c, c.player, typeof c.player.getPlayerState === 'function']);
      if (!ok) return false;
      const st = c.player.getPlayerState();
      return st === 3;
    } catch (_) {
      return false;
    }
  ...
  reset(c) {
    c.lastRecoveryTime = Date.now();
    _state.stats.recoveries += 1;
  },
  retry(c) {
    c.lastRecoveryTime = Date.now();
    _state.stats.recoveries += 1;
...
function applyAdapter(a) {
  if (!a) return;
  const keys = ['isBuffering', 'isPaused', 'reset', 'retry', 'requestPlay'];
 ...
 * ============================================================================ */
function canAct(c, now) {
  const conds = [!c.lastRecoveryTime, typeof c.lastRecoveryTime === 'number' ? now - c.lastRecoveryTime > _runtime.cooldownMs : true];
  return anyTrue(conds);
}
function computeAllowedPause(c) {
  if (c && typeof c.expectedPauseMs === 'number') {
    return c.expectedPauseMs;
  }
  return 0;
}
function applyBufferingRule(c, now) {
  const bufThreshold = rndInt(_runtime.thresholds.bufferingMinMs, _runtime.thresholds.bufferingMaxMs);
  ...
  if (allTrue(conds)) {
    log(`[${ts()}] 🛠️ Watchdog -> Player ${c.index + 1} BUFFERING>${bufThreshold}ms → reset`);
    _adapter.reset(c);
    return true;
  }
  return false;
}
function applyPausedRule(c, now) {
  const allowed = computeAllowedPause(c);
  const extra = rndInt(_runtime.thresholds.pausedMinExtraMs, _runtime.thresholds.pausedMaxExtraMs);
  const threshold = allowed + extra;
  ...
  if (allTrue(conds)) {
    log(`[${ts()}] 🛠️ Watchdog -> Player ${c.index + 1} PAUSED>${threshold}ms → retry`);
    _adapter.retry(c);
    return true;
  }
  return false;
...
function loopOnce() {
  const now = Date.now();
  _state.stats.loops++;
  _state.stats.lastRun = now;
  for (let i = 0; i < _state.controllers.length; i++) {
    const c = _state.controllers[i];
    if (typeof c.index !== 'number') c.index = i;
    const didBuf = applyBufferingRule(c, now);
    const didPaused = applyPausedRule(c, now);
    if (anyTrue([didBuf, didPaused])) {
    }
  }
}
function scheduleNext(delay) {
  clearTimers();
  let d = typeof delay === 'number' ? delay : _runtime.loop.baseMs + rndInt(_runtime.loop.jitterMinMs, _runtime.loop.jitterMaxMs);
...
function tick() {
  ...
  try {
    ...
    try {
      log(`[${ts()}] ❗ Watchdog Error: ${String(err && err.message ? err.message : err)}`);
    } catch (_) {}
  }
