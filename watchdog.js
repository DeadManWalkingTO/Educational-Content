 ...
 * Συμμορφώνεται με τις πολιτικές του project (ESM imports, SemVer, χωρίς ||/&&).
 */
...
function canFireAutoNext(ctrl, required, played) {
  ...
  try {
    ...
    if (ok === true) {
      const diff = Date.now() - ctrl.lastSeekAt;
  ...
  try {
    ...
    const ok = allTrue(parts);
    if (ok === true) {
      const diff = Date.now() - ctrl.lastPausedStart;
  ...
  try {
    ...
    const ok = allTrue(parts);
    if (ok === true) {
      const elapsed = (Date.now() - ctrl.playingStart) / 1000;
  ...
  try {
    ...
    if (canFire === true) {
      if (ctrl.watchtimeFired !== true) {
        ...
        try {
          if (typeof ctrl.clearTimers === 'function') {
            ctrl.clearTimers();
          }
        } catch (_) {}
