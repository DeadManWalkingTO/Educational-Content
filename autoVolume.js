 ...
 * Αλλαγές: Χρήση utils.whenPlayingAndUnmuted αντί τοπικού helper
 *         + clamp για όρια έντασης.
 */
...
/* ========================= Imports ========================= */
import { scheduleSafe, rndInt, allTrue, isNumber, clamp, makeLogger, whenPlayingAndUnmuted } from './utils.js';
...
/**
 * Προγραμματισμός αλλαγών έντασης (freeze-aware).
 * @param {any} player
 * @param {{volumeChangeChance:number, volumeRange:number[]}} cfg
 * @param {number} durationSec
 * @param {string} group
 * @param {any} ctrl - PlayerController (για freeze-softTasks)
 */
export function scheduleVolumeChanges(player, cfg, durationSec, group = 'pc:volume', ctrl = null) {
  ...
  while (i < planned) {
    ...
    scheduleSafe(
      () => {
        whenPlayingAndUnmuted(player, ctrl, () => _applyVolume(player, rangeArr), 800, 2000, group, 'volume-change');
      },
      delayMs,
      group,
      'volume-change'
...
/**
 * Micro-adjust κοντά στο τέλος (≥ 600s), freeze-aware.
 */
export function scheduleMicroAdjust(player, durationSec, group = 'pc:volume', ctrl = null) {
  ...
  scheduleSafe(
    () => {
      whenPlayingAndUnmuted(player, ctrl, microTask, 800, 2000, group, 'volume-micro');
    },
    microDelayMs,
    group,
    'volume-micro'
