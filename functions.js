...
// functions.js — Version v3.5.5
// Updates:
// - loadAltList now loads from local random.txt
// - If Alt list has <10 IDs, it is ignored (empty array returned)
// - All previous features preserved
// ==========================
// GLOBAL CONSTANTS
// ==========================
const JS_VERSION = "v3.5.5";
...
function rndDelayMs(minS, maxS) { return rndInt(minS * 1000, maxS * 1000); }

// Update stats panel UI
function updateStatsPanel() {
    const p = document.getElementById("statsPanel");
    if (!p) return;
    p.textContent = `📊 Stats — AutoNext:${stats.autoNext} \\
...
// NEW: Load Alt list from random.txt
async function loadAltList() {
    try {
        const res = await fetch('random.txt', { cache: 'no-store' });
        if (res && res.ok) {
            const text = await res.text();
            ...
            if (ids.length >= 10) {
                ...
                console.warn(`[${ts()}] Alt list ignored (only ${ids.length} IDs)`);
                return [];
            }
        }
    } catch (err) {
        console.warn(`[${ts()}] Alt list load error: ${err}`);
    }
    return [];
}
...
function initPlayersDynamic() {
    ...
    for (let i = 0; i < totalPlayers; i++) {
        ...
        const vid = (videoListMain && videoListMain.length > 0)
            ...
            : (videoListAlt && videoListAlt.length > 0
                ? videoListAlt[i % videoListAlt.length]
                : internalList[i % internalList.length]);
...
function destroyPlayers() {
    ...
    players.forEach((p, i) => {
        try { clearPlayerTimers(i); if (p && typeof p.destroy === 'function') p.destroy(); } catch (err) { }
    });
    players = [];
    playerProfiles = [];
    playerTimers = [];
...
function scheduleRandomPauses(p, i, profile) {
    ...
    const t1 = setTimeout(() => {
        try {
            ...
            const r = setTimeout(() => { try { p.playVideo(); } catch (_) { } }, rndDelayMs(1, 3));
    ...
    const t2 = setTimeout(() => {
        try {
            ...
            const r2 = setTimeout(() => { try { p.playVideo(); } catch (_) { } }, rndDelayMs(2, 6));
