 ...
 * Τροποποίηση: Τυχαιότητα στο capSec (ανώτατο όριο required watch-time) με range + bias + profile tuning.
 * - Base range: 10–20 min (600–1200 s) + bias ±30 s.
 * - Profile tuning: focused +60 s, explorer -60 s.
 * - Clamp: [480 s, 1500 s].
 */
...
function _computeCapSec(profileName) {
  // Base range: 10–20 min
  const baseMinSec = 10 * 60; // 600
  const baseMaxSec = 20 * 60; // 1200
  // Τυχαία επιλογή εντός του range (ακέραια δευτερόλεπτα)
  ...
  // Ασφαλές clamp
  cap = clamp(cap, 480, 1500); // μεταξύ 8 και 25 λεπτών
...
export function getRequiredWatchTime(durationSec, profileName = 'unknown') {
  ...
  // ΝΕΟ: τυχαιότητα στο capSec με βάση το profile
  const capSec = _computeCapSec(profileName);

  // Εύρος ποσοστών ανά διάρκεια (όπως πριν)
  let minPct = 0.55;
  let maxPct = 0.75;
  if (d < 120) {
    minPct = 0.92;
  ...
  // Debug log (Activity Panel μέσω utils.log)
  try {
...
function _getUnmutePlan(durationSec, profileName, baseStartDelaySec) {
  ...
  // Πιθανό εύρος έντασης για unmute
  let vLo = 10;
  let vHi = 30;
  if (d < 120) {
    vLo = 15;
