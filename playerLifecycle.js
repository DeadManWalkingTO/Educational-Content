...
/* ========================= Imports ========================= */
import { isDefined, allTrue, isFunction, scheduleSafe, rndInt, getPlayerScope, makeLogger } from './utils.js';
import { getOrigin, getYouTubeEmbedHost, MIN_WATCH_TIME } from './globals.js';
...
export function recreateWithPrewarm(ctrl, videoId, opts = {}) {
  ...
  const minMs = typeof opts?.waitMinMs === 'number' ? opts.waitMinMs : 8000;
  const maxMs = typeof opts?.waitMaxMs === 'number' ? opts.waitMaxMs : 12000;
...
export function fullCleanControllerStrict(ctrl) {
  // 1) Clear schedulers / timers (controller API)
  try {
    const canClear = [];
  ...
  // 2) Καθαρισμός PRE-WARM (player + inner) πριν από το hardDestroy/purge
  try {
    const hasPw = allTrue([isDefined(ctrl?._prewarm) === true]);
    if (hasPw === true) {
      // 2a) Αφαίρεση prewarm inner
      try {
  ...
  // 3) Hard destroy (κύριος player) + μηδενισμός _innerId
  try {
    hardDestroy(ctrl);
  } catch (_) {}

  // 4) Purge container (αφαίρεση ΟΛΩΝ των child nodes)
  try {
    const cid = getContainerId(ctrl);
    purgeContainer(cid);
  } catch (_) {}

  // 5) Reset flags/state baseline
  try {
    resetFlags(ctrl);
  } catch (_) {}
}
