...
export class Watchdog {
  ...
  _emit(event, ...args) {
    ...
    if (typeof fn === 'function') {
      try {
        ...
        this.stats.errors++;
        this._log('error', 'Handler error', e);
      }
    }
  }
  ...
  _jitterDelay() {
    ...
    let minMs = 120;
    let maxMs = 300;
    if (typeof j.minMs === 'number') {
      minMs = j.minMs;
    }
    if (typeof j.maxMs === 'number') {
      maxMs = j.maxMs;
    }
  ...
  async _tick() {
    ...
    if (!hasCB) {
      this._log('warn', 'onRequestPlay callback missing');
      this._scheduleTick();
      return;
    }
  ...
  _backoffAndSchedule() {
    let minMs = 500;
    let maxMs = 1200;
    if (typeof this.options.backoffMsMin === 'number') {
      minMs = this.options.backoffMsMin;
    }
    if (typeof this.options.backoffMsMax === 'number') {
      maxMs = this.options.backoffMsMax;
    }
    const r = this._random01();
