...
- humanMode.js → v4.11.26 — Διατήρηση μίας δήλωσης για **chainStarted, **totalPlayers, \_\_setTotalPlayers(), startFirstPlayerNow().
- Remove duplicate const declarations (MIN/MAX_CHAIN_DELAY_SEC, MIN/MAX_WARMUP_SEC) in humanMode.js.
- humanMode.js → v4.11.25 — Καθαρισμός διπλών δηλώσεων και patch bump.
- Fix Unexpected token '} in humanMode.js:
- humanMode.js — Αφαίρεση stray closing brace πριν το End Of File, ισορροπία {{}} OK.
- Fix SyntaxError: Illegal return statement in humanMode.js:
- humanMode.js → v4.11.24 — Αφαίρεση διπλότυπων/ορφανών blocks (notifyPlayerStarted), καθαρισμός top-level return, συντακτικό OK.
- Warmup 5–10s before start for chain-delayed players:
- humanMode.js → v4.11.23 — Εμφάνιση (createPlayerIfNeeded) warmup 5–10s πριν από το προγραμματισμένο start.
- Chain-delayed appearance & start:
  -humanMode.js → v4.11.22 — Προστέθηκε αλυσίδα καθυστέρησης: MIN 60s, MAX 180s. startFirstPlayerNow(), notifyPlayerStarted().
- playerController.js → v6.21.39 — Ειδοποίηση προς humanMode στην έναρξη (χωρίς &&).
- main.js → v3.33.9 — Αύξηση έκδοσης (συμβατότητα).
...
2025-12-23 | playerController.js | v6.21.17 → v6.21.18 | Simplified YouTube state mapping (YT*STATES, stateToName), EOF marker.
...
Converted timers: 13
2025-12-23 | playerController.js | v6.21.14 → v6.21.15 | Phase-1: add hasFn/callIfFn, add Unmute+Retry helpers + pendingUnmute handler, add local pc*\_ guards, use globals.hasArrayWithItems; const seekSec.
2025-12-23 | playerController.js | 6.21.14 → 6.21.15 | Phase-1: add local guards (pc\_\_), add hasFn/callIfFn, unify Unmute+Retry, use globals.hasArrayWithItems; minor const seekSec.
