...
export function scheduleMidSeek(ctrl, overrideMs) {
  ...
  try {
    ...
    const shouldStop = allTrue([isShort === true]) || allTrue([isLate === true]);
    if (shouldStop === true) {
      const reason = isShort === true ? `short-window=${wmFirst?.windowSec ?? 0}s` : `late timeLeft=${wmFirst?.timeLeft ?? 0}s`;
      log(`⏹️ Player ${ctrl.index + 1} Mid-Seek → stop (${reason})`);
      return;
    }
  } catch (_) {}
